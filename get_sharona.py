    ...
    for i in range(data["iterations"]):
        ...
        def get_childs_num(childs_rand, people_per_household):
            if childs_rand <= data["1 childs"] and people_per_household >= 2:
                ...
                return 3
            elif childs_rand < data["household with chd"]:
                return people_per_household - 2
            else:
                return None
...
def statistic(df1):
    ...
    emp_count = round((df1['employed'].value_counts() / data["iterations"] * 100), 2)
    ax4 = fig.add_subplot(gs[1, 1])
    emp_bar = emp_count.plot(kind='bar', rot=0, ax=ax4)
    ...
    cars_p = df1["cars#"].value_counts() / data["iterations"] * 100
